...
def _fmt(amount: int) -> str:
    """금액 변화를 +/- 부호가 붙은 문자열로 표시 (예: +150,000원 / -80,000원)"""
...
def _resolve_landing(game: dict, discord_id: str, events: list):
    ...
    if ttype == "city":
        ...
        if owner is None:
            ...
            events.append(
                f"🏙️ **{tile['name']}** (Lv.{level}) - {owner_name}님 소유! "
                f"통행료 {_fmt(-rent)} (→ {owner_name}님 {_fmt(rent)})"
            )

    elif ttype == "tax":
        amount = tile["amount"]
        _pay(game, discord_id, amount, to_fund=True)
        events.append(f"💸 **{tile['name']}**: {_fmt(-amount)} (사회복지기금 적립)")
        ...
        game["fund"] = 0
        p["cash"] += amount
        events.append(f"🎉 사회복지기금 {_fmt(amount)} 획득! (전액 수령)")
...
def _apply_card(game: dict, discord_id: str, card: dict, events: list):
    ...
    if ctype == "gain":
        ...
        if value > 0 and new_pos < old_pos:
            p["cash"] += GO_BONUS
            events.append(f"➡️ 출발점을 통과하여 {_fmt(GO_BONUS)}!")
        p["position"] = new_pos
        ...
        for other_id in game["order"]:
            if other_id != discord_id and not game["players"][other_id]["bankrupt"]:
                ...
                n += 1
        events.append(f"{_fmt(value * n)} (플레이어 {n}명에게 각 {value:,}원씩)")
    elif ctype == "pay_all":
        n = 0
        for other_id in game["order"]:
            if other_id != discord_id and not game["players"][other_id]["bankrupt"]:
...
def roll_dice(game: dict, discord_id: str):
    ...
    if new_pos < old_pos:
        p["cash"] += GO_BONUS
        events.append(f"🏁 출발점을 통과하여 {_fmt(GO_BONUS)}!")
    p["position"] = new_pos
...
def buy_property(game: dict, discord_id: str):
    ...
    msg = f"**{tile['name']}** 구매! {_fmt(-tile['price'])}"
...
def build(game: dict, discord_id: str, levels: int = 1):
    ...
    return True, f"**{tile['name']}**을(를) Lv.{cur_level + levels}(으)로 건설! {_fmt(-cost)}"
...
def escape_island(game: dict, discord_id: str):
    ...
    p["island_turns"] = 0
    return True, f"💰 무인도에서 탈출했습니다! {_fmt(-ISLAND_ESCAPE_FEE)}"
