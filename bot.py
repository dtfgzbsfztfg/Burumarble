...
from game import engine, storage, board_render
...
async def roll(interaction: discord.Interaction):
    ...
    elif result and result.get("needs_decision"):
        embed.add_field(name="구매 결정 대기중", value="`/구매` 또는 `/턴종료`로 결정해주세요.", inline=False)
    else:
...
@bot.tree.command(name="턴종료", description="빈 땅을 구매하지 않고 턴을 넘깁니다.")
async def pass_turn(interaction: discord.Interaction):
    game = _get_game(interaction.channel_id)
    if not game:
        await interaction.response.send_message(embed=_embed("⚠️ 오류", "진행중인 게임이 없습니다.", COLOR_ERR))
        return
    ok, msg = engine.end_turn(game, interaction.user.id)
    _save(game)
    await interaction.response.send_message(embed=_embed("턴 종료" if ok else "⚠️ 오류", msg, COLOR if ok else COLOR_ERR))
...
@bot.tree.command(name="보드", description="보드 전체 소유 현황과 말 위치를 이미지로 봅니다.")
async def board(interaction: discord.Interaction):
    ...
    svg_text = board_render.render_board_svg(game)
    svg_file = discord.File(io.BytesIO(svg_text.encode("utf-8")), filename="board.svg")
    embed.set_footer(text="🖼️ board.svg 첨부파일을 다운로드해서 보드 이미지를 확인하세요 (디스코드가 SVG 미리보기를 지원하지 않아요)")
    await interaction.response.send_message(embed=embed, file=svg_file)
